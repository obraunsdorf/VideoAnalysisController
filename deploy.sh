...
REPONAME="VideoAnalysisController"
...
then
    ...
    then
        ...
        ./github-release release -s ${GITHUB_TOKEN} -u ${USERNAME} -r ${REPONAME} -t ${GIT_TAG_FULL} -c ${GIT_HASH} --draft -d "This is the release of VideoAnalysisController ${GIT_TAG_FULL} </br>CHANGELOG:</br>$CHANGELOG"
    fi
